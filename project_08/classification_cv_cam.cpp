...
void get_input_data_cv(cv::Mat &image_file, float* input_data, int img_h, int img_w, const float* mean, float scale)
{
...
void show_usage()
{
    ...
    std::cout << "\nmobilenet example: \n"
              << "    ./classification_cv -m /path/to/mobilenet.tmfile -l /path/to/labels.txt -i /path/to/img.jpg -g "
                 "224,224 -s 0.017 -w 104.007,116.669,122.679"
              << std::endl;
}
...
int main(int argc, char* argv[])
{
    ...
    if(!cap.isOpened())
    {
        std::cout << "Cannot open a camera" << std::endl;
        return -4;
    }
    for(;;)
    {
        ...
        get_input_data_cv(frame, ( float* )input_tensor.data, img_h, img_w, _channel_mean, 0.017);
